        ...
        auth.createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    ...
                    callback(true, "Registration Successs", "${auth.currentUser?.uid}")
...
//        val id = ref.push().key.toString() to auto generate ids for product
//        .child().child() bhaneko herirachy bhaneko parent ani child child users database ko bhitra bhitra
        ref.child(userId).setValue(model)
            .addOnCompleteListener {
                if (it.isSuccessful){
                    callback(true, "Registration Successs")
                }else{
                    callback(false, "${it.exception?.message}")
                }
            }
    ...
    override fun forgetPassword(
       ...
       auth.sendPasswordResetEmail(email)
           .addOnCompleteListener {
               if (it.isSuccessful){
                   callback(true, "Reset link send to $email")
               }else{
                   callback(false, "${it.exception?.message}")
               }
           }
